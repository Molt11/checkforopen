...
function seedGatewayFromEnv(dbConn: Database.Database): void {
  ...
  try {
    dbConn.prepare(`
      INSERT INTO gateways (name, host, port, token, is_primary, status)
      VALUES (?, ?, ?, ?, 1, 'online')
    `).run(name, host, port, token)
    logger.info({ name, host, port }, 'Seeded default gateway from environment')
  } catch (err) {
    logger.error({ err }, 'Failed to seed default gateway')
  }
}
